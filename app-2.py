...
Fragment API - fragment_api_lib kutubxonasi
"""
...
def do_fragment_sync(order):
    try:
        ...
        client = FragmentAPIClient(
            seed=WALLET_MNEMONIC if WALLET_MNEMONIC else None,
            fragment_cookies=FRAGMENT_COOKIES if FRAGMENT_COOKIES else None,
        )
        ...
        if isinstance(r, dict):
            return r.get("ok") or r.get("success") or r.get("status") == "ok"
        return bool(r)
