...
export default function Premium({ open, onClose, settings, showToast, uid, tgUser, onBalanceUpdate }) {
  ...
  async function buy() {
    ...
    try {
      const r = await fetch('/api/buy', {
        ...
        body: JSON.stringify({
          uid, service:'premium', username,
          months, price: total, promo
        })
      })
      const d = await r.json()
      if (d.success) {
        showToast(`✅ Premium ${months} oy yuborildi!`, 'ok')
  ...
  return (
    <FullPage open={open} onClose={onClose} title="👑 Premium Gift">
      ...
      <RecipientSelector value={recip} onChange={setRecip} usernameVal={uname} onUsernameChange={setUname} color="purple" />

      <PromoInput uid={uid} product={pkMap[months]} onApply={(d,c) => { setDisc(d); setPromo(c) }} />
